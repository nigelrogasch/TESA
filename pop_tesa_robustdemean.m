...
function [EEG, com] = pop_tesa_robustdemean(EEG, timeWindow, thresholdSD, excludeWindow)
...
if nargin < 2
    ...
    geometry = {1 [1 0.3] [1 0.3] [1 0.3]};

    uilist = {{'style', 'text', 'string', 'Robust demean','fontweight','bold'} ...
    ...
    try
        ...
        excludeWindow = str2num(result{3}); %#ok<ST2NM>
...
% -------------------------------
fprintf('Running robust demeaning...\n');
...
% -------------------------------
if isempty(excludeWindow)
    com = sprintf('EEG = pop_tesa_robustdemean(EEG, [%g %g], %g);', ...
        timeWindow(1), timeWindow(2), thresholdSD);
else
    com = sprintf('EEG = pop_tesa_robustdemean(EEG, [%g %g], %g, [%g %g]);', ...
        timeWindow(1), timeWindow(2), thresholdSD, excludeWindow(1), excludeWindow(2));
end

fprintf('Demeaning complete.\n');
