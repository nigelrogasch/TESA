...
function EEG = tesa_pcasupress( EEG, timeWin )
...
if size(timeWin,2) ~= 2
	error('Please provide two time values for the supression window: [start, end]');
end

%Check that time values given for supression window are in range of data
if timeWin(1,1) < EEG.times(1,1) || timeWin(1,2) > EEG.times(1,end)
    error('Time values for the supression window are out of data range. Note that time values are in ms.');
end
...
%Check that the supression window does not over lap with removed data
