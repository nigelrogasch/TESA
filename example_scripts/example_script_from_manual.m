...
%Step 7: Epoch data (-1000 to 1000 ms)
EEG = pop_epoch( EEG, {  'R128'  }, [-1  1], 'epochinfo', 'yes');

%Step 8: Baseline correct (-500 ms to -10 ms)
EEG = pop_rmbase( EEG, [-500  -10]);

%Step 9: Remove TMS pulse artifact (-2 to 10 ms)
EEG = pop_tesa_removedata( EEG, [-2 10] );

%Step 10: Interpolate removed data
EEG = pop_tesa_interpdata( EEG, 'linear' );

%Step 11: Downsample data (5000 Hz to 1000 Hz)
EEG = pop_resample( EEG, 1000);

%Step 12: Remove bad trials including a 
...
%Step 13: Remove interpolated data (-2 to 10 ms)
EEG = pop_tesa_removedata( EEG, [-2 10] ); 

%Step 14: Remove TMS-evoked muscle activity (using FastICA and auto component selection)
...
%Step 15: Extend data removal to 15 ms (-2 to 15 ms)
EEG = pop_tesa_removedata( EEG, [-2 15] );

%Step 16: Interpolate removed data
EEG = pop_tesa_interpdata( EEG, 'linear' );

%Step 17: Bandpass (1-100 Hz) and bandstop (48-52 Hz) filter data
...
%Step 19: Interpolate missing channels
...
%Step 22: Output peak analysis in table
